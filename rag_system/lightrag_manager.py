...
class LightRAGManager:
    ...
    def add_document(self, document_content: Dict[str, Any]) -> str:
        ...
        Returns:
            Document ID
        """
        logger.info(f"Adding document: {document_content['metadata']['title']}")
        ...
        try:
            ...
            logger.info(f"Added document {document_id} to RAG")
            ...
            if self.file_manager:
                try:
                    ...
                    logger.info(f"Document {document_id} uploaded to OpenAI with file ID {file_id} and assistant ID {assistant_id}")
                except Exception as e:
    ...
    def query(self, query_text: str, document_id: Optional[str] = None, mode: str = "hybrid") -> Dict[str, Any]:
        """
        ...
        if is_location_query:
            logger.info(f"Detected location query: {location_info}")
            return self._handle_location_query(query_text, location_info, document_id)
        
        # Regular content query
        try:
            # Try to use OpenAI file manager if available and document_id is provided
            if self.file_manager and document_id and document_id in self.document_metadata:
                # Check if we have an assistant ID for this document
                if "openai_assistant_id" in self.document_metadata[document_id]:
                    logger.debug(f"Using OpenAI assistant for document {document_id}")
                    
                    # Query the document using OpenAI assistant
                    assistant_result = self.file_manager.query_document(document_id, query_text)
                    
                    if assistant_result:
                        logger.debug(f"OpenAI assistant result: {assistant_result[:200]}..." if len(assistant_result) > 200 else assistant_result)
                        return {
                            "query": query_text,
                        ...
                        logger.warning(f"OpenAI assistant query failed, falling back to RAG")
            
            # Fall back to RAG if OpenAI file manager is not available or failed
            # Check if we're using LightRAG or SimpleRAG
            if hasattr(self.rag, 'query'):
                # SimpleRAG
                logger.debug(f"Using SimpleRAG to query: {query_text}")
                result = self.rag.query(query_text)
                logger.debug(f"SimpleRAG query result: {result[:200]}..." if len(result) > 200 else result)
            else:
                # LightRAG
                logger.debug(f"Using LightRAG to query: {query_text} with mode: {mode}")
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                ...
                logger.debug(f"LightRAG query result: {result[:200]}..." if len(result) > 200 else result)
...
class SimpleRAG:
        ...
        if os.path.exists(embeddings_path):
            try:
                with open(embeddings_path, 'r') as f:
                    ...
                    self.embeddings = {
                        tuple(eval(k)): np.array(v) for k, v in data.get("embeddings", {}).items()
                    }
            except Exception as e:
    ...
    def insert(self, text: str, source_id: str) -> None:
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                ...
                print(f"Error generating embedding for chunk {i} of document {source_id}: {e}")
    ...
    def query(self, query_text: str, param: Any = None) -> str:
        """
        Query the RAG system.
        ...
        if not results:
            # If no results, use the first document
